...
function RegistrationForm(){
    ...
    const handleSubmit = async (formData) => {
        try {
            let response = await axios.post(`http://localhost:3002/events/${id}/register-volunteer`, 
                { 
                    userId: user._id, 
                    name: formData.name, 
    ...
    return (
        <div className='row top-conteiner' style={{ padding:"110px 6%",background:`linear-gradient(rgba(255, 255, 255, 0.8), rgba(255, 255, 255, 0.7)), url(${image})`,backgroundRepeat:"no-repeat",backgroundSize:"cover"}}>
            ...
            transition: "all 0.3s ease-in-out" }}>
                <h2 className="text-center" style={{color:"darkgreen",fontSize:"32px",fontWeight:"bold"}}>Register As Volunteer</h2>
                <FormComponent fields={fields} initialState={initialState} onSubmit={handleSubmit} buttonText="Register" />
            </div>
        </div>
    );
}
